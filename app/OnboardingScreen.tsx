...
export default function OnboardingScreen() {
  ...
  const babyWeights: { [key: number]: number } = {
    8: 0.01, 10: 0.04, 12: 0.014, 14: 0.043, 16: 0.1, 18: 0.19,
    20: 0.3, 22: 0.43, 24: 0.6, 26: 0.76, 28: 1.0, 30: 1.3,
    32: 1.7, 34: 2.1, 36: 2.6, 38: 3.0, 40: 3.4
  };
  ...
  const calculateBabyWeight = (week: number) => {
    ...
    for (let i = 0; i < weeks.length - 1; i++) {
      if (week >= weeks[i] && week <= weeks[i+1]) {
        ...
        const v1 = babyWeights[w1], v2 = babyWeights[w2];
        return +(v1 + ((week - w1)/(w2 - w1))*(v2 - v1)).toFixed(2); // 2 décimales
      }
    }
  ...
  const handleNext = () => {
    if (step === 1 && (!nom || !prenom || !age)) return Alert.alert('Erreur', 'Veuillez remplir tous les champs'), null;
    if (step === 1) return setStep(2);
    if (step === 2 && (!poids || !taille || !poidsAvantGrossesse)) return Alert.alert('Erreur', 'Veuillez remplir tous les champs'), null;
    if (step === 2) return setStep(3);
  };

  const handleFinish = async () => {
    if (!semaineGrossesse || !dateAccouchement) return Alert.alert('Erreur', 'Veuillez remplir tous les champs');
    const weekNum = parseInt(semaineGrossesse);
    if (weekNum < 1 || weekNum > 42) return Alert.alert('Erreur', 'La semaine de grossesse doit être entre 1 et 42');
    if (!auth.currentUser) return Alert.alert('Erreur', 'Utilisateur non connecté');
  ...
  const renderStep2 = () => (
    <View style={styles.stepContainer}>
      <View style={styles.iconCircle}><Ionicons name="fitness-outline" size={40} color="#C4ABDC" /></View>
      <Text style={styles.stepTitle}>Informations physiques</Text>
      <Text style={styles.stepSubtitle}>Pour un suivi personnalisé</Text>

      <View style={styles.inputContainer}><Text style={styles.label}>Poids actuel (kg)</Text>
      ...
      <View style={styles.inputContainer}><Text style={styles.label}>Taille (cm)</Text>
      ...
      <View style={styles.inputContainer}><Text style={styles.label}>Poids avant grossesse (kg)</Text>
  ...
  const renderStep3 = () => (
    <View style={styles.stepContainer}>
      <View style={styles.iconCircle}><Ionicons name="body-outline" size={40} color="#C4ABDC" /></View>
      <Text style={styles.stepTitle}>Informations de grossesse</Text>
      <Text style={styles.stepSubtitle}>Dernière étape !</Text>

      <View style={styles.inputContainer}><Text style={styles.label}>Semaine de grossesse actuelle</Text>
      ...
      <View style={styles.inputContainer}><Text style={styles.label}>Date prévue d'accouchement</Text>
        <TextInput style={styles.input} placeholder="JJ/MM/AAAA" placeholderTextColor="#C4ABDC" value={dateAccouchement} onChangeText={setDateAccouchement} />
        <Text style={styles.helperText}>Format: JJ/MM/AAAA</Text>
      </View>

      {semaineGrossesse && parseInt(semaineGrossesse) >= 1 && parseInt(semaineGrossesse) <= 42 && (
        <View style={styles.previewCard}>
          <Text style={styles.previewTitle}>Aperçu</Text>
          <View style={styles.previewRow}><Text style={styles.previewLabel}>Trimestre:</Text>
            <Text style={styles.previewValue}>{calculateTrimester(parseInt(semaineGrossesse))}</Text>
          </View>
          <View style={styles.previewRow}><Text style={styles.previewLabel}>Poids bébé estimé:</Text>
            <Text style={styles.previewValue}>{calculateBabyWeight(parseInt(semaineGrossesse))} kg</Text>
          </View>
        </View>
      )}
    </View>
      ...
      <LinearGradient colors={['#1B0E20','#2A1A35','#1B0E20']} style={styles.gradient}>
        ...
        >
          ...
          <View style={styles.buttonContainer}>
            {step>1 && <TouchableOpacity style={styles.backButton} onPress={handleBack} activeOpacity={0.8}>
              <Ionicons name="arrow-back" size={20} color="#C4ABDC"/>
              <Text style={styles.backButtonText}>Retour</Text>
            </TouchableOpacity>}
            <TouchableOpacity style={[styles.nextButton,step===1&&styles.nextButtonFull]} onPress={step===3?handleFinish:handleNext} activeOpacity={0.9}>
              <LinearGradient colors={['#C4ABDC','#9B88D3','#876BB8']} start={[0,0]} end={[1,1]} style={styles.nextButtonGradient}>
...
const styles = StyleSheet.create({
  ...
  gradient: { flex: 1 },
  scrollContent: { flexGrow: 1, padding: 30, paddingTop: 60, paddingBottom: 170 }, // espace pour boutons
  ...
  iconCircle: { width: 80, height: 80, borderRadius: 40, backgroundColor: 'rgba(196, 171, 220, 0.15)', justifyContent: 'center', alignItems: 'center', alignSelf: 'center', marginBottom: 10, borderWidth: 2, borderColor: 'rgba(196, 171, 220, 0.3)' },
  ...
  input: { 
    ...
    borderRadius: 80,  // borderRadius ajusté
    padding: 16, 
    fontSize: 16, 
    color: '#FFFFFF', 
  ...
  previewCard: { backgroundColor: 'rgba(196, 171, 220, 0.15)', borderRadius: 16, padding: 20, marginTop: -5, borderWidth: 1, borderColor: 'rgba(196, 171, 220, 0.3)' },
  ...
  backButton: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: 16, borderRadius: 80, borderWidth: 2, borderColor: '#C4ABDC', gap: 8 },
